...
**Definition of Done:** Both rule specs have been read in full; exactly one PRD is locked; a subject name is derived; the PRD heading outline is in the task output; the locked context block has been printed.
...
- If no PRD was supplied at invocation, ask the user which PRD to translate. Do not guess from workspace contents.
...
- Read the locked PRD completely. Capture every section and subsection heading, in document order, with the PRD's own numbering. Record the outline in the task output.

- Apply *The whole PRD is in scope* from the rules spec. If the user already supplied an exclusion, record it verbatim with who asked. Do not invent an exclusion, and do not ask for one.
